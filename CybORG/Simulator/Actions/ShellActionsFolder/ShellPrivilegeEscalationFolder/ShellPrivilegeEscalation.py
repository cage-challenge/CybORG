    ...
    def run_web_server(self, session: Session):
        for file in session.hostname.files:
            if file.file_type == FileType.PYTHON:
                session.hostname.add_process(name="python", ppid=session.process.pid, user=session.user, open_ports={"Port": 8080, "Application Protocol": "HTTP", "Address": "broadcast"}, process_type="webserver", version="python SimpleHTTPServer")
            ...
            if file.file_type == FileType.GCC:
                if file.check_executable(session.user):
                    if code_file.check_readable(session.user):
                        if code_file.file_type == FileType.DirtyCowCode and file.version == FileVersion.U4_2_4_1 and session.hostname.os_type == OperatingSystemType.LINUX:
                            exe_file = copy.deepcopy(code_file)
                            exe_file.file_type = FileType.DirtyCowExe
                            exe_file.name = "40839"
                            session.hostname.files.append(exe_file)
                            ...
                            return exe_file
                        elif code_file.file_type == FileType.UDEV141CODE and session.hostname.os_type == OperatingSystemType.LINUX and file.version == FileVersion.U4_2_4_1:
                            exe_file = copy.deepcopy(code_file)
