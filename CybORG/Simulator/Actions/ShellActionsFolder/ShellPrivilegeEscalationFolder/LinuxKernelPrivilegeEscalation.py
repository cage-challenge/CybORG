...
class LinuxKernelPrivilegeEscalation(ShellPrivilegeEscalation):
    ...
    def execute(self, state: State):
        ...
        attacker_host = attacker_session.hostname
        ...
        attacker_session.hostname.files.append(run_file)
        ...
        attacker_ip = None
        target_ip = None
        if attacker_session.hostname == target_session.hostname:
            attacker_ip = IPv4Address("127.0.0.1")
            target_ip = IPv4Address("127.0.0.1")
        else:
            for interface in attacker_session.hostname.interfaces:
                if interface.name != "lo":
                    for interface2 in target_session.hostname.interfaces:
        ...
        nc = attacker_session.hostname.get_file("nc")
        ...
        if nc is not None:
            if nc.check_executable(attacker_session.user):
                nc_process = attacker_session.hostname.add_process(ppid=attacker_session.process.pid,
        ...
        if executable_file.check_executable(target_session.user) and nc_process is not None:
            if run_file.check_executable(target_session.hostname.get_user('root')):
                new_session = state.add_session(user='root', agent=self.agent, host=target_session.hostname.hostname, parent=None)
                self.obs.add_session_info(hostid="hostid1", session_type="shell", timeout=0,
                                          session_id=new_session.ident, agent=self.agent)
                new_ephemeral_port = target_session.hostname.get_ephemeral_port()
