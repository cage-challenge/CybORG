...
class ServicePersistenceWindows(MSFAction):
    ...
    def execute(self, state):
        ...
        if session_to_persist.hostname.os_type != OperatingSystemType.WINDOWS:
            return obs

        # find shared subnet of the two hosts
        server_interface = None
        per_interface = None
        for persist_interface in session_to_persist.hostname.interfaces:
        ...
        new_session = state.add_session(host=session_to_persist.hostname.hostname, agent=self.agent,
        ...
        service_name = ''.join(state.np_random.choice(list(string.ascii_uppercase + string.ascii_lowercase)) for _ in range(5))
        session_to_persist.hostname.add_service(service_name=service_name, process=process, session=session_to_persist)
