...
class MeterpreterPS(MeterpreterAction):
    ...
    def execute(self, state: State):
        ...
        users = []
        proc_sh = session.hostname.add_process(name="sh", user=session.user, path="/bin/")
        proc_ps = session.hostname.add_process(name="ps", user=session.user)
        ...
        if session.hostname.os_type == OperatingSystemType.LINUX:
            ...
            for group in session.user.groups:
                ...
                # is a user with minimal privileges?
                    ...
                    for proc in session.hostname.processes:
            ...
            obs.add_system_info(hostid="0", architecture=session.hostname.architecture)
            if root:
                state.remove_process(host=session.hostname.hostname, pid=proc_sh.pid)
                state.remove_process(host=session.hostname.hostname, pid=proc_ps.pid)
                for proc in session.hostname.processes:
