...
def test_MS17_010_killchain_sim_expected_output(cyborg_scenario1):
    cyborg = cyborg_scenario1
    agent = 'Red'
    initial_result = cyborg.get_observation(agent)
...
def test_with_reboot():
    ...
    agent = 'Red'
    initial_obs = cyborg.get_observation(agent)
    target_hostname = 'Gateway'
    target_address = initial_obs[target_hostname]['Interface'][0]['IP Address']
...
# Another killchain process.
def test_son_MS17_010_killchain():
    path = str(inspect.getfile(CybORG))
    path = path[:-7] + f'/Simulator/Scenarios/scenario_files/Scenario1.yaml'
    sg = FileReaderScenarioGenerator(path)
    ...
    initial_obs = cyborg.get_observation(agent)
    # create ssh session on pretend pi host
    session = initial_obs['Attacker']['Sessions'][0]['ID']
    k_ip_address = initial_obs['Attacker']['Interface'][0]['IP Address']
    pp_ip_address = initial_obs['Gateway']['Interface'][0]['IP Address']
    action = MSFPortscan(session=session, agent=agent, ip_address=pp_ip_address)
